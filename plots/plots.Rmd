...
For the plot, I rename `vegan` to `plant-based` to be consistent with study 3, and also reorder the factor to go from meat to vegetarian to plant-based.
```{r reorder_study1}
study1 <-
  study1 %>% 
  mutate(
...
## 1.4 Word clouds
To get a better idea of the features that the labels contained, I also create word clouds.
The `food_labels.csv` data set contains the foods, their features, and the supermarket they come from.
```{r get_features}
# get data set
features <- 
  read_csv(
...
I want to create three word clouds, one for each type of food.
In the data set, there's no identifier of the food type.
However, the food type is in the `study1` data set.
So I first merge those two to get a food identifier for each food in `features`.
```{r merge_features_analyis_file}
features <- 
  left_join(
    features,
    study1 %>% 
...
For the wordcloud, we only need a frequency table of the features.
I first only select the feature column and then create three frequency tables.
```{r frequency_tables}
features <- 
  features %>% 
  select(feature, food_type) %>% 
  group_by(food_type) %>% 
  count(feature) %>% 
  ungroup()
```

Get the wordclouds per food type.
Start with meat-based.
```{r wordcloud_meat}
wordcloud_meat <-
  features %>% 
  filter(food_type == "Meat-based") %>% 
  select(-food_type) %>% 
  wordcloud2(
    .,
    size = 3,
...
Then vegetarian.
```{r wordcloud_vegetarian}
wordcloud_vegetarian <-
  features %>% 
  filter(food_type == "Vegetarian") %>% 
  select(-food_type) %>% 
  wordcloud2(
    .,
    size = 3,
...
And plant-based.
```{r wordcloud_plant_based}
wordcloud_plant_based <-
  features %>% 
  filter(food_type == "Plant-based") %>% 
  select(-food_type) %>% 
  wordcloud2(
    .,
    size = 3,
...
Next, I load the final data files from Study 2 that I saved at the end of the analysis report (`analysis_report_study2.Rmd`).
```{r get_data_study2}
study2 <- read_csv(here("data", "study2", "analysis_file.csv"))
desire <- read_csv(here("data", "study2", "desire.csv"))
...
## 2.2 Raincloud plots
For the paper, I'll need two plots: one raincloud plot that shows the four different labels types per food type, faceted by measure (i.e., desire vs. simulations); and one interaction plot that shows the interaction between intention to reduce eating meat and desire per food type.
I'll start with the raincloud plot.
Actually, a 4x2 plot plus facet will probably be much too large, so I'll do one per measure.

First recode factor like above.
```{r numeric_factor_study2}
study2 <- 
  study2 %>% 
  mutate(
    label_type_n = case_when(
      label_type == "neutral" ~ 1,
      label_type == "contextual" ~ 2,
      label_type == "sensory" ~ 3,
      label_type == "health_positive" ~ 4
    ),
    label_type_n = as.factor(label_type_n)
  )
```

Then I start with the desire plot.
...
Also, because we present tests of the main effect of `label_type` exclusively for one hypothesis, I'll add the average of the two food types as a separate line.
```{r get_summary_desire_study3}
# aggregate first
aggregated_desire_study2 <- 
  study2 %>%
...
# then get summary stats per label type and food type
summary_stats_desire_study2 <- summarySEwithin(
  data = aggregated_desire_study2,
  measurevar = "rating1",
...
# get the overall trend aggregated over food
aggregated_desire2_study2 <- 
  study2 %>% 
  filter(measure == "desire") %>% 
...
# get summary stats per label type
summary_stats_desire2_study2 <- summarySEwithin(
  data = aggregated_desire2_study2,
  measurevar = "rating1",
  withinvars = c("label_type"),
  idvar = "pp"
) %>% 
  mutate(
    label_type_n = case_when(
...
# add those together
summary_stats_all_desire_study2 <- 
  bind_rows(
    summary_stats_desire_study2,
    summary_stats_desire2_study2
  )
```

Alright, now the raincloud plot.
Hm, not too sure about the overall line.
```{r make_it_rain_desire_study2}
rc_plot_desire_study2 <-
  ggplot(
    study2 %>%
      filter(measure == "desire"),
    aes(
  ...
  geom_point(aes(x = as.numeric(label_type_n)-.2, # transfer the factor to number so that I can move the rain to the left with the -.2
             ...
             alpha = 0.4) +
  geom_point( # the mean
    data = summary_stats_all_desire_study2,
    aes(
  ...
  labs(fill = "Food Type", # otherwise I get two legends
       color = "Food Type") +
  scale_x_discrete(labels = c("Neutral", "Context", "Sensory", "Health-positive")) + # rename the x axis after transforming it to numeric earlier
...
Next, get summary stats for simulations.
```{r get_summary_simulations_study3}
# aggregate first
aggregated_simulations_study2 <- 
  study2 %>%
  filter(measure == "simulations") %>% 
  group_by(pp, label_type, food_type) %>% 
  summarize(
    rating3 = mean(rating3)
  )

# then get summary stats per label type and food type
summary_stats_simulations_study2 <- summarySEwithin(
  data = aggregated_simulations_study2,
  measurevar = "rating3",
  withinvars = c("label_type", "food_type"),
  idvar = "pp"
) %>% 
...
# get the overall trend aggregated over food
aggregated_simulations2_study2 <- 
  study2 %>% 
  filter(measure == "simulations") %>% 
  group_by(pp, label_type) %>% 
  summarize(
...
# get summary stats per label type
summary_stats_simulations2_study2 <- summarySEwithin(
  data = aggregated_simulations2_study2,
  measurevar = "rating3",
  withinvars = c("label_type"),
  idvar = "pp"
) %>% 
...
# add those together
summary_stats_all_simulations_study2 <- 
  bind_rows(
    summary_stats_simulations_study2,
    summary_stats_simulations2_study2
  )
```

Alright, now the raincloud plot.
```{r make_it_rain_simulations_study2}
rc_plot_simulations_study2 <-
  ggplot(
    study2 %>%
      filter(measure == "simulations"),
    aes(
  ...
  geom_point(aes(x = as.numeric(label_type_n)-.2, # transfer the factor to number so that I can move the rain to the left with the -.2
             ...
             alpha = 0.4) +
  geom_point( # the mean
    data = summary_stats_all_simulations_study2,
    aes(
      x = as.numeric(label_type_n)-.25, # same as above, moving it a bit to the left to be right between cloud and rain
      y = rating3,
      color = food_type
...
Next, I plot the interaction between categorical and numerical variables.
...
```{r interaction_plots_simple_study2}
...
```{r interaction_model_mixed_effects_study2}
# get models
load(here("plots", "models", "models_study2.RData"))

# the plot with the effects package
plot(
  effect("food_type:meat_per_week_s", 
         h4_food_meat_model),
         ci.style = "bands",
  multiline = TRUE)
```

Plots for the interaction of meat-eating frequency and food type.
```{r meat_frequency_interaction_study2}
# get the data set
h4_food_meat_effects <-
  get_effects(
...
Next, I load the final data files from Study 3.
I saved those at the end of the analysis report (`analysis_report_study3.Rmd`).
```{r get_data_study3}
study3 <- read_csv(here("data", "study3", "analysis_file.csv"))
attractiveness <- read_csv(here("data", "study3", "attractiveness.csv"))
simulations_study3 <- read_csv(here("data", "study3", "simulations.csv"))
...
# then get summary stats
summary_stats_study3 <- summarySEwithin(
  data = aggregated_study3,
  measurevar = "rating",
...
rc_plot_study3 <-
  ...
  labs(fill = "Food Type", # otherwise I get two legends
       color = "Food Type") +
  scale_x_discrete(labels = c('Control','Simulation-based')) + # rename the x axis after transforming it to numeric earlier
  scale_color_manual(values = c("#009E73", "#D55E00")) +
  scale_fill_manual(values = c("#009E73", "#D55E00")) +
  theme_cowplot() +
  theme(
...
```{r change_diet_food_type_interaction_study3}
# get models
load(here("plots", "models", "models_study3.RData"))
...
```{r meat_frequency_food_type_interaction_study3}
# get the data set
exp_meat_effects <- 
  get_effects(
...
```{r meat_frequency_label_type_interaction_study3}
# get the data set
...
Also note that the wordclouds don't have a clean code solution for saving yet, see [here](https://github.com/Lchiffon/wordcloud2/issues/8).
Those I just saved from the viewer pane manually.
```{r save_plots, eval=F}
...
# study 2 raincloud desire
tiff(here("plots", "figure4.tiff"), width = 879*8, height = 632*8, units = "px", res = 600)
rc_plot_desire_study2
dev.off()

# study 2 raincloud simulations
tiff(here("plots", "figure5.tiff"), width = 879*8, height = 632*8, units = "px", res = 600)
rc_plot_simulations_study2
dev.off()

# study 2 interaction plot
tiff(here("plots", "figure6.tiff"), width = 879*8, height = 632*8, units = "px", res = 600)
h4_food_meat_plot
dev.off()

# study 3 raincloud
tiff(here("plots", "figure7.tiff"), width = 879*8, height = 632*8, units = "px", res = 600)
rc_plot_study3
dev.off()

# study 3 interaction h4
ggsave(here("plots", "figure8.tiff"),
       plot = h4_plot,
       dpi = 600)

# study 3 exploratory plot 1
ggsave(here("plots", "figure9.tiff"),
       plot = exp_meat_plot,
       dpi = 600)

# study 3 exploratory plot 2
ggsave(here("plots", "figure10.tiff"),
