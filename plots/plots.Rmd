...
rc_plot <- function(
  ...
  p <-
    ...
    ylab(ylab) +
    xlab("Food Type") +
    scale_x_discrete(labels = c("Meat-based", "Vegan","Vegatarian")) + # rename the x axis after transforming it to numeric earlier
...
# helper function that combines the effects tibbles from the models for attractiveness and simulations, so that I can facet wrap later
merge_effects <-
  function(
    attr_dat,
    sim_dat
...
consumption_plot <-
  rc_plot(study1,
          ...
          "food_type_n",
          "food_type",
          "Proportion of Consumption Features")
...
non_consumption_plot <-
  rc_plot(study1,
          ...
          "food_type_n",
          "food_type",
          "Proportion of Non-Consumption Features")
...
Also, I need summary stats, this time with with-person standard error because there are multiple measurements of proportions per food type.
...
study1_long <-
  ...
  mutate( # for raincloud
    ...
    proportion_type = recode(
      ...
      "non_consumption" = "Non-consumption",
...
summary_stats_study1 <-
  ...
  mutate( # for raincloud
    proportion_type_n = case_when(
      proportion_type == "Consumption" ~ 1,
      proportion_type == "Non-consumption" ~ 2,
...
I rcode the factor just like above.
...
Usually I prefer this "raw" approach, but it's not entirely accurate to represent the simple slope analyses we conducted.
...
# get the data set
exp_meat_effects <- merge_effects(
  get_effects(
...
# get the data set
exp_meat_label_effects <- merge_effects(
  get_effects(
...
```{r save_plots, eval=T}
# study1 sensory
ggsave(here("plots", "sensory_raincloud.tiff"),
       plot = sensory_plot,
       dpi = 600)

# study1 consumption
ggsave(here("plots", "consumption_raincloud.tiff"),
       plot = consumption_plot,
       dpi = 600)

# study1 non-consumption
ggsave(here("plots", "non_consumption_raincloud.tiff"),
       plot = non_consumption_plot,
       dpi = 600)

# study1 situation independent
ggsave(here("plots", "situation_independent_raincloud.tiff"),
       plot = situation_independent_plot,
       dpi = 600)

# study1 by food type
tiff(here("plots", "food_type_raincloud.tiff"), width = 879*8, height = 632*5, units = "px", res = 600)
food_type_plot
dev.off()

# study 2 raincloud
tiff(here("plots", "study2_raincloud.tiff"), width = 879*8, height = 632*8, units = "px", res = 600)
rc_plot_study2
dev.off()

# study 2 interaction h4
ggsave(here("plots", "h4_plot.tiff"),
       plot = h4_plot,
       dpi = 600)

# study 2 exploratory plot 1
ggsave(here("plots", "frequency_meat_food_type.tiff"),
       plot = exp_meat_plot,
       dpi = 600)

# study 2 exploratory plot 2
ggsave(here("plots", "frequency_meat_food_labels.tiff"),
