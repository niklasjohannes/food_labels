...
However, I don't think this would be appropriate here: a) the residuals are normally distributed; b) I would argue that a 100 rating slider is conceptually rather different than a proportion (I wouldn't know how to count successes and failures for a binomial distribution). 
...
sum(abs(resid(h1_sim, scaled = TRUE)) > 2) / length(resid(h1_sim))
sum(abs(resid(h1_sim, scaled = TRUE)) > 2.5) / length(resid(h1_sim))
sum(abs(resid(h1_sim, scaled = TRUE)) > 3) / length(resid(h1_sim))
...
# proportion of outliers
sum(abs(resid(h1_attr, scaled = TRUE)) > 2) / length(resid(h1_attr))
sum(abs(resid(h1_attr, scaled = TRUE)) > 2.5) / length(resid(h1_attr))
sum(abs(resid(h1_attr, scaled = TRUE)) > 3) / length(resid(h1_attr))
