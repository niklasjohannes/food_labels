---
title: "Data Analysis Report Study 3"
...
There's two participants who indicated to be vegetarian, even though they said they were omnivores at the beginning of the study. 
One of those participants even eats meat once a week.
I'll check whether their exclusion (`pp_30` and `pp_63`) changes anything for the hypthesized analyses later.
```{r describe_diet}
# diet
...
For the supplementary online materials, we also want to provide a table that lists all food descriptions with average attractiveness ratings for each label type.
For this, I get the means per food by label type.
```{r label_per_food_means}
means_per_food <-
  working_file %>% 
  filter(measure == "attractiveness") %>% 
...
Next, I check whether excluding the two self-declared vegetarians changes the results.
The results are robust.
```{r h1_exclude_vegetarians}
# simulations
...
As a last robustness check, I see whether gender influences the effect of label type on the two outcomes.
It does not.
```{r h1_gender}
...
Next, we want to know whether simulation ratings interact with either of the factors.
...
Simulations predicted attractiveness.
As an another exploratory analysis, I'll check whether the reverse also holds.
With the `bobyqa` optimizer, the model converges without any warnings.
All effects are significant.
```{r h3_attr_simulations}
# set contrasts
options(contrasts = c("contr.sum", "contr.poly"))
...
The preregistered hypothesis reads as follows:
>The intention to reduce meat will be associated positively with attractiveness ratings for the plant-based foods, and negatively with desire (sic: attractiveness) for meat-based foods.
...
I check that with attractiveness.
Indeed, the more meat people eat, they find plant-based foods attractive, but less so for enhanced food labels.
...
I'd also be interested whether a feeling of hunger interacts with food type when predicting attractiveness.
It does such that for meat-based foods an increase in hunger is associated with higher attractiveness, whereas the opposite is the case for plant-based foods.
```{r expl_hunger_food_labels_attr}
# scale hunger
attractiveness <-
  attractiveness %>% 
...
If indeed there is a cultural preference for meat when people are hungry (aka when I'm hunger I want a proper meal, and meat is a proper meal), then this could also be reflected in attitudes toward different foods.
Thus, I'll inspect whether hunger predicts the attitudes for the three food types.
Looking at the plot, it does not; this is backed up by the raw correlations.
```{r exp_hunger_attitudes}
# scatterplots
ggplot(working_file %>% 
...
It was interesting that the effect of label type was not significant anymore after including simulations as predictor.
Below I want to see whether this also happens when predicting simulations with attractiveness.
These tests follow the logic of the classic Sobel test.
...
These patterns are consistent with a conditional independence of attractiveness of label type, conditional on knowing simulations.
```{r sobel_tests}
...
Next, I conduct a formal mediation analysis with the `mediation::mediate` command.
For some reason, `mediate` doesn't take an individual-level factor as predicor, so I have to transform the `label_type` factor to numerical.
Not a big problem, because we only have two levels.
Also, `mediate` doesn't work with crossed-level factors, meaning that we need to remove the random effects of `food`.

We can see that the effect of `label_type` on attractiveness is fully mediated by simulations, as the average causal mediation effect is large (larger than the total effect, because the direct effect is negative) and significant, whereas the direct effect is small and nonsignificant.
```{r sim_attr_mediation}
...
wide_data <- 
  wide_data %>% 
  mutate(
    label_type_n = if_else(label_type == "control", 0, 1)
  )
...
# model predicting outcome
out_fit <- 
  lme4::lmer(
    attractiveness_rating ~
      label_type_n + simulation_rating +
      (1 + label_type_n + simulation_rating | pp),
    wide_data
  )
...
save(
  ...
  file = here("plots", "models", "models_study3.RData")
)
